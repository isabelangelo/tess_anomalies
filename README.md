...
1. `generate_tce_data.py`: Downloads TCE .xml files and queries files to generate TCE tables with relevant DV metrics.
*xml files are saved to corresponding sector directories (e.g., `./s0036/`, `./s0037/`, etc.). 
*TCE tables are saved to `./data/tce_data/`.

2. `create_tce_batches.py`: Sorts TCEs, and generates batches of TCE tables for follow up that are ordered by priority. 
